...
def predict(threshold, decision_scores):
    """
    Convert One Class SVMs decision scores into binary anomaly predictions.

    Parameters
    threshold (float): Decision score threshold. 
    decision_scores (np.ndarray):: Decision function outputs from the One Class SVMs.
...
def main():
    ...
    with open(report_path, "w") as f:
        f.write("One-Class SVM Evaluation Report\n")
        f.write("=" * 35 + "\n\n")
