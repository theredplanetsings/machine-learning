...
def predict(threshold, decision_scores):
    """
    Convert Isolation Forest decision scores into binary anomaly predictions.

    Parameters
    threshold (float): Decision score threshold. 
    decision_scores (np.ndarray):: Decision function outputs from the Isolation Forest.
...
def isolation_forest(channel_data, channel, report_path, results):
    """
    Train and evaluate a Isolation Forest for a single telemetry channel.
    ...
    df.to_csv(f"results/data_frame_format/isolation_forest.csv")
...
def main():
    """
    Run Isolation Forest evaluation across all telemetry channels and save a consolidated report.
    """
    results = {}
    report_path = f"results/text_format/isolation_forest.txt"
    channels = load_channels()

    with open(report_path, "w") as f:
        f.write("Isolation Forest Evaluation Report\n")
        f.write("=" * 35 + "\n\n")
    
    standard_fitted_data = load_data(channels)
    for channel in standard_fitted_data:
        isolation_forest(standard_fitted_data[channel], channel, report_path, results)
    
    print("[✓] Predicted all telemetry anomaly data with isolation forest")
